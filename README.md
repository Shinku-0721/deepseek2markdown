...
- 全量导出设置固定并发上限与请求间隔，并逐会话写入 ZIP；单个会话请求失败时继续处理其余会话。
...
- `storage`：保存思考过程、网络搜索和分支展示选项。
- `https://chat.deepseek.com/*`：请求 DeepSeek 的会话、历史记录和公开分享接口。

登录凭证仅保存在当前 DeepSeek 标签页的内存中，不写入 `chrome.storage`。导出内容在浏览器本地整理和压缩，扩展不包含后端服务，也不会把对话发送给开发者或其他自建服务。

## 注意事项

- 全量导出依赖 DeepSeek 当前网页接口，DeepSeek 更新接口后可能需要同步适配。
- 页面刷新或扩展重新加载后，需要打开任意 DeepSeek 对话，让页面发起一次请求后才能执行全量导出。
- 网络异常或接口限流会自动重试；多次失败的单个会话会计入失败数量，其余会话仍会继续导出。
