...
function createMarkdownModule() {
  ...
  /**
   * 将一个会话渲染为主文档、上传文件和去重后的 Search 附件集合。
   *
  ...
  function createMarkdownExport(session, settings) {
    ...
    const attachments = Array.isArray(settings.attachments) ? settings.attachments : [];
    ...
    if (session?.export_error) {
      lines.push(...renderCallout(
        'failure',
        '导出失败',
        inlineText(session.export_error, '会话历史获取失败'),
      ), '');
    }
    ...
    return {
      ...
      files: [{
        ...
        mimeType: 'text/markdown;charset=utf-8',
      }, ...searchFiles.values(), ...attachments
        .filter(attachment => attachment.path && attachment.content != null)
        .map(attachment => ({
  ...
  /**
   * 在主文档开头写入上传文件链接，并显式展示未能获取的附件。
   *
   * @param {string[]} lines 主文档行数组。
   * @param {object[]} attachments 已规范化的附件描述。
   */
  function appendAttachmentSection(lines, attachments) {
    ...
    for (const attachment of attachments) {
      if (attachment.path && attachment.content != null) {
        lines.push(`- [${escapeLinkText(attachment.label)}](${encodeLinkPath(attachment.path)})`);
      } else {
        const label = inlineText(attachment.label, '未命名附件');
        const error = inlineText(attachment.error, '获取失败');
        lines.push(`- ${label}（获取失败：${error}）`);
      }
    }
    lines.push('');
  }
  ...
  /** @param {*} value ZIP 内相对路径。 */
  function encodeLinkPath(value) {
