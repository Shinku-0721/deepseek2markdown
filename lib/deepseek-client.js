...
function createClientModule() {
  ...
  function createDeepSeekClient(environment = {}) {
     ...
     * @param {object} [selection] 可选的附件 ID/遍历位置白名单，仅用于刷新后重试受影响文件。
    ...
    async function fetchUploadedFiles(session, selection = {}) {
      ...
      const selectedFileIds = Array.isArray(selection.fileIds)
        ...
        : null;
      const selectedAttachmentIndexes = Array.isArray(selection.attachmentIndexes)
        ? new Set(selection.attachmentIndexes)
        : null;
      const hasSelection = selectedFileIds !== null || selectedAttachmentIndexes !== null;
      const attachments = collectUploadedFiles(session);

      for (const [attachmentIndex, attachment] of attachments.entries()) {
        const fileId = attachment.id == null ? null : String(attachment.id);
        const isSelected = fileId != null
          ? selectedFileIds?.has(fileId)
          : selectedAttachmentIndexes?.has(attachmentIndex);
