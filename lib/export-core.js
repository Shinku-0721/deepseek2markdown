...
function createExportCore(markdownExport) {
  ...
  function prepareMarkdownAttachments(attachments) {
    ...
    attachments.forEach((attachment, index) => {
      ...
      const fileName = uniqueFileName(sanitizeFileName(originalName), usedNames);
      prepared.push({
  ...
  /**
   * 在大小写不敏感语义下生成不重复文件名，并把序号插入扩展名前。
   *
   * @param {string} baseName 已清理的基础文件名。
   * @param {Set<string>} usedNames 已占用的小写文件名集合。
   * @returns {string} 当前集合中唯一的文件名。
   */
