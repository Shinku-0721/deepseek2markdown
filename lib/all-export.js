...
function createAllExportModule() {
  ...
  function createAllExport(adapters = {}) {
    ...
    async function run(format, options = {}, onProgress) {
      ...
      /** 让普通历史请求和附件凭据刷新共享同一个全局启动间隔与请求计数。 */
      const fetchHistoryFromNetwork = async (sessionId) => {
        ...
        counts.networkCount++;
        return deepSeekClient.getHistoryData(sessionId);
      };
      ...
      try {
        ...
        /**
         * 缓存附件凭据被拒绝时刷新一次历史，并只重试具有稳定 ID 的受影响附件。
         *
        ...
        const refreshRejectedAttachments = async (entry, attachments) => {
          const refreshableIds = [...new Set(attachments
            .filter(file => file?.refreshable && file.id != null)
            .map(file => String(file.id)))];
          if (!entry.cacheHit || refreshableIds.length === 0) {
          ...
          try {
            ...
            const retried = await deepSeekClient.fetchUploadedFiles(session, {
              fileIds: refreshableIds,
            });
            const retriesById = new Map(retried.map(file => [String(file.id), file]));
            return {
              session,
              attachments: attachments.map(file => {
                const fileId = file?.id == null ? null : String(file.id);
                if (!fileId || !refreshableIds.includes(fileId)) return file;
                return retriesById.get(fileId) || {
                  ...file,
